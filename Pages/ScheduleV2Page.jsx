...
import {
  applyShiftHoursForward, bookingV2Keys, createDatedShift, createRepeatingShift, createTimeOff,
  ...
  timeOffCoversDate, updateDatedShift, updateRepeatingShift, updateTimeOff,
...
const SHIFT_ACTION_ITEMS = [{ key: "edit", label: "Edit this day", icon: <FiEdit3 /> }, { key: "repeating", label: "Set repeating shifts", icon: <FiRepeat /> }, { key: "timeoff", label: "Add time-off", icon: <FiUmbrella /> }, { type: "divider" }, { key: "delete", label: "Delete this shift", icon: <FiTrash2 />, danger: true }];
...
function ShiftDialog({ open, onClose, onSaved, onPartial, staff, staffId, date, timezone, editing }) {
  const [form] = Form.useForm();
  const [saving, setSaving] = useState(false);
  const [failures, setFailures] = useState([]);
  const selectedStaffId = editing ? staffIdOf(editing) : staffId ?? staff[0]?.id;
  const initial = useMemo(() => ({ staff_id: selectedStaffId, date: editing ? dayjs(editing.date || String(editing.starts_at).slice(0, 10)) : date, timezone: editing?.timezone || timezone, times: [{ start_time: dayjs(`2000-01-01T${timePart(editing?.start_time || editing?.starts_at) || "09:00"}`), end_time: dayjs(`2000-01-01T${timePart(editing?.end_time || editing?.ends_at) || "17:00"}`) }], notes: editing?.notes || "" }), [date, editing, selectedStaffId, timezone]);
  useEffect(() => { if (open) { form.resetFields(); form.setFieldsValue(initial); setFailures([]); } }, [form, initial, open]);
  async function submit(values) {
    ...
    if (editing) {
      try {
        await updateDatedShift(editing.id, payloads[0]);
        message.success("Shift updated.");
        form.resetFields();
        onSaved();
      } catch (error) {
        setFailures([apiMessage(error, "The shift could not be updated.")]);
      } finally {
        setSaving(false);
      }
      return;
  ...
  return <Modal title={editing ? "Edit this day" : "Add dated shift"} open={open} onCancel={onClose} footer={null} destroyOnHidden><Form form={form} layout="vertical" initialValues={initial} onFinish={submit} clearOnDestroy>
    ...
    <Form.List name="times">{(fields, { add, remove }) => <>{fields.map((field, index) => <div className="shift-time-row" key={field.key}><Form.Item name={[field.name, "start_time"]} label={index ? undefined : "Starts"} rules={[{ required: true }]}><TimePicker format="HH:mm" minuteStep={15} /></Form.Item><Form.Item name={[field.name, "end_time"]} label={index ? undefined : "Ends"} rules={[{ required: true }]}><TimePicker format="HH:mm" minuteStep={15} /></Form.Item>{fields.length > 1 && <Button aria-label="Remove time" icon={<FiTrash2 />} onClick={() => remove(field.name)} />}</div>)}{!editing && <Button type="dashed" icon={<FiPlus />} onClick={() => add({ start_time: dayjs("2000-01-01T09:00"), end_time: dayjs("2000-01-01T17:00") })}>Add another time</Button>}</>}</Form.List>
    <Form.Item name="notes" label="Notes"><Input.TextArea rows={2} /></Form.Item><Form.Item name="timezone" hidden><Input /></Form.Item><div className="shift-modal-actions"><Button onClick={onClose}>Cancel</Button><Button type="primary" htmlType="submit" loading={saving}>{editing ? "Save changes" : "Save shift"}</Button></div>
  </Form></Modal>;
}
...
function ShiftBlocks({ state, canEdit, onAdd, onEdit, onAction, person, date }) {
  ...
  return state.working.map((row) => canEdit
    ? <Dropdown key={row.id} trigger={["hover", "click"]} menu={{ items: SHIFT_ACTION_ITEMS, onClick: ({ key }) => onAction(key, { row, person, date }) }}><button type="button" className={`shift-block ${row.kind}`}><span className="shift-block__type">{row.kind === "repeating" ? <><FiRepeat /> Repeating</> : "Dated shift"}</span><strong>{clock(row.start_time || row.starts_at)}–{clock(row.end_time || row.ends_at)}</strong>{row.notes && <small>{row.notes}</small>}</button></Dropdown>
    : <div className={`shift-block ${row.kind}`} key={row.id}><span className="shift-block__type">{row.kind === "repeating" ? <><FiRepeat /> Repeating</> : "Dated shift"}</span><strong>{clock(row.start_time || row.starts_at)}–{clock(row.end_time || row.ends_at)}</strong>{row.notes && <small>{row.notes}</small>}</div>);
}
...
export default function ScheduleV2Page() {
  ...
  const [prefill, setPrefill] = useState({ staffId: null, date: dayjs(), fromCalendar: false, sourceShift: null });
  ...
  const openAdd = (key, personId = staffFilter === "all" ? staff[0]?.id : staffFilter, date = cursor) => { setEditing(null); setPrefill({ staffId: personId, date, fromCalendar: false, sourceShift: null }); setDialog(key); };
  const openCalendarAdd = (key, personId, date) => { setEditing(null); setPrefill({ staffId: personId, date, fromCalendar: true, sourceShift: null }); setDialog(key); };
  const openEdit = (kind, record) => { setEditing(record); setDialog(kind); };
  const handleShiftAction = (action, selection) => {
    const { row, person, date } = selection;
    if (action === "delete") return remove(row.kind, row.kind === "repeating" ? row.rule : row);
    if (action === "edit") {
      setEditing(row.kind === "repeating" ? row.rule : row);
      setPrefill({ staffId: person.id, date, fromCalendar: true, sourceShift: null });
      setDialog(row.kind === "repeating" ? "repeating" : "dated");
      return;
    }
    setEditing(null);
    setPrefill({ staffId: person.id, date, fromCalendar: true, sourceShift: action === "repeating" ? row : null });
    setDialog(action);
  };
  ...
  return <main className="shift-studio">
    ...
    <section className="shift-workspace"><div className="shift-toolbar"><div className="shift-view-switch">{["day","week","month"].map((item) => <button className={view === item ? "active" : ""} key={item} onClick={() => setView(item)}>{item}</button>)}</div><div className="shift-nav"><Button icon={<FiChevronLeft />} aria-label={`Previous ${step}`} onClick={() => setCursor(cursor.subtract(1, step))} /><button className="shift-nav__today" onClick={() => setCursor(dayjs())}>Today</button><DatePicker className="shift-nav__date" aria-label="Jump to date" allowClear={false} inputReadOnly value={cursor} format={() => title} onChange={(date) => date && setCursor(date)} suffixIcon={<FiCalendar />} variant="borderless" /><Button icon={<FiChevronRight />} aria-label={`Next ${step}`} onClick={() => setCursor(cursor.add(1, step))} /></div><div className="shift-toolbar__actions"><Select value={staffFilter} onChange={setStaffFilter} options={[{ value: "all", label: "Everyone" }, ...staff.map((person) => ({ value: person.id, label: staffName(person) }))]} />{canEdit && <Dropdown menu={{ items: ADD_ITEMS, onClick: ({ key }) => openAdd(key) }}><Button type="primary" icon={<FiPlus />}>Add</Button></Dropdown>}</div></div>
      ...
      {loading ? <div className="shift-roster-loading"><Skeleton active paragraph={{ rows: 8 }} /></div> : error ? <Alert type="error" showIcon message="The staff schedule could not be loaded" description={apiMessage(error, "Refresh and try again.")} /> : view === "month" ? <MonthView dates={dates} cursor={cursor} staff={shownStaff} shifts={shiftsQ.data || []} repeating={repeatingQ.data || []} timeOffs={timeOffQ.data || []} onAdd={openAdd} /> : <Roster dates={dates} staff={shownStaff} shifts={shiftsQ.data || []} repeating={repeatingQ.data || []} timeOffs={timeOffQ.data || []} view={view} canEdit={canEdit} onAdd={openCalendarAdd} onEdit={openEdit} onAction={handleShiftAction} />}
    </section>
    <Rules staff={staff} repeating={repeatingQ.data || []} timeOffs={timeOffQ.data || []} canEdit={canEdit} onEdit={openEdit} onDelete={remove} />
    <ShiftDialog open={dialog === "dated"} onClose={() => setDialog(null)} onSaved={saved} onPartial={invalidate} staff={staff} staffId={prefill.staffId} date={prefill.date} timezone={timezone} editing={editing} />
    <RepeatingDialog open={dialog === "repeating"} onClose={() => setDialog(null)} onSaved={saved} staff={staff} staffId={prefill.staffId} date={prefill.fromCalendar ? prefill.date : null} timezone={timezone} editing={editing} sourceShift={prefill.sourceShift} />
